...
namespace Stars
{
    ...
    public static class RG_Utils
    {
        ...
        public static string globalLanguage = "English";

        public static string _imagePath = "/image";
 ...
 "file://" + _editordownloadpath + "/";
    ...
    "stlwin";
        ...
        /// <summary>
        /// 从1970.1.1到当前经过的毫秒数转化为本地DateTime,为各地通用起见，尽量避免使用 clime
        /// </summary>
        /// <param name="t"></param>
        /// <returns></returns>
        public static DateTime msToDateTime(ulong t)
        {
            DateTime begin = new DateTime(1970, 1, 1);
            DateTime dt = begin.Add(new TimeSpan((long)t * TimeSpan.TicksPerMillisecond));
            return dt.ToLocalTime();
        }
        ...
        /// 从下载目录读取二进制文件
        /// </summary>
        /// <param name="fileName">下载目录的相对位置</param>
        /// <returns></returns>
        static public byte[] readFromDownloadPath(string fileName)
        {
        ...
        /// 字符串转Color
        /// </summary>
        /// <param name="colStr">需要转换的字符串,数值为0-255</param>
        /// <returns></returns>
        public static Color GetColorByString(string colStr)
        {
            if (colStr.Length <= 0)
                return Color.white;
            string tempStr = colStr;
            tempStr = tempStr.Replace("(", "").Replace(")", "");
            string[] tmp_sValues = tempStr.Trim(' ').Split(',');
            if (tmp_sValues != null && tmp_sValues.Length == 3)
            {
                float tmp_fX = float.Parse(tmp_sValues[0]) / 255;
                float tmp_fY = float.Parse(tmp_sValues[1]) / 255;
                float tmp_fZ = float.Parse(tmp_sValues[2]) / 255;

                return new Color(tmp_fX, tmp_fY, tmp_fZ);
            }
            return Color.white;
        }
        ...
        /// </summary>
        /// <returns>
        /// NetworkReachability.ReachableViaCarrierDataNetwork  →  "当前为运行商网络（2g、3g、4g）"
        /// NetworkReachability.ReachableViaLocalAreaNetwork  →  "当前为Wifi网络"
        /// NetworkReachability.NotReachable  → "没有连接网络"
        /// </returns>
        ...
        /// <returns></returns>
        public static uint getBMILv(float bmi)
        {
            if (bmi < 18.5)
            {
                return 1;
        ...
        /// 计算bmi
        /// </summary>
        /// <param name="BodyHeight">身高，米为单位</param>
        /// <param name="BodyWeight">体重，公斤为单位</param>
        /// <returns></returns>
        public static float calBMI(float BodyHeight,float BodyWeight)
        {
        ...
        public static T[] GetComponentsOnlyInChildren<T>(this Transform transform)
        {
            T[] r = null;
            if (transform != null)
            {
                r = new T[transform.childCount];
                for (int i = 0; i < transform.childCount; i++)
                {
                    r[i] = transform.GetChild(i).GetComponent<T>();
                }
            }
            return r;
        }
        ...
        {
            return  Environment.CommandLine.Contains("Publish.commandLineBuild");
        }

      
        public static string getDownloadImagePath()
        {
            if (!Directory.Exists(getDownloadPath()+_imagePath))
            {
                Directory.CreateDirectory(getDownloadPath()+ _imagePath);
            }
            return getDownloadPath() + _imagePath;
        }
